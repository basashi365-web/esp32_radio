...
- 2026-06-06: 18650内蔵案を検討した。2S直列は電源余裕があるが、2セル用の充電制御/BMSが手元にないため今回は保留する。
- 1S2P並列は心理的に避けたい方針。今回の試作では使わない。
- 手持ち部品では、緑TP4056 USB-C基板とXL6009E1昇圧モジュールを候補にする。緑TP4056は写真上で `B+` / `B-` / `OUT+` / `OUT-` があり、`DW01A` / `8205A` 保護回路付きに見えるため、青TP4056より今回向き。
- この構成はpower pathではない。充電しながら鳴らす常用は避け、まずは電源OFFで充電、使用時は充電USBを外す運用で試す。
- OLEDへ電池残量%を出す場合は、昇圧後の5Vではなく18650側を抵抗分圧してADCで読む。
- 2026-06-06: 1S Li-ion充放電統合モジュール Type-C / 5V 2A をAliExpressで5個購入した。到着予定は2026-06-10から2026-06-12。購入時スクリーンショットは `D:\data\codex\shared_assets\hardware\esp32_radio\20260606_battery_power_module` に保存した。
- 商品説明上は、充電しながらの放電に対応、昇圧出力5Vから5.15V / 2A、負荷50mA未満継続で出力OFF、K端子で出力ON/OFF可能。到着後は、再生中の5V低下、USB抜き差し時の瞬断、stopped待機中の自動OFF、発熱を確認する。
- 2026-06-07: TP4056 + XL6009E1を筐体内へ配置し、デバイス動作を確認した。GPIO1へ100kΩ/100kΩ分圧を入れ、OLED右下へ `Bxx%` として18650残量目安を表示するファームウェアをCOM24へ書き込んだ。
- 電源投入時に起動しているようでもOLEDが点灯しないことがあるため、OLED初期化を3回試行し、起動後も未検出なら3秒ごとに再初期化する処理を追加した。
- バッテリー表示は実機で動作確認できた。GPIO1とGNDの間に10uFセラミックコンデンサを追加したところ、コンデンサなしに比べてADC由来の表示ふらつきがかなり減り、残量表示が実用上落ち着いた。表示更新を約7秒ごと、1%以内の変化は前回表示維持にして、OLED表示のちらつきをさらに抑える方針にした。
- 2026-06-07: バッテリー運用版として完成。USBケーブルから解放されることで、ラジオデバイスとしての使い心地がかなり良くなった。完成写真は `D:\data\codex\shared_assets\hardware\esp32_radio\20260607_completed_battery_radio` に保存した。写真上では WNYC FM / Vol 3 / battery 53% / `stream ok` が表示されている。
- 完成後の筐体改善点: 電源スイッチ部分は実機では超音波カッターで後加工して穴を開けたため、次回筐体設計では最初から電源スイッチ穴を入れる。USB充電できるように、TP4056または将来の充放電統合モジュールのUSB充電口用の穴も最初から入れる。OLED右下がスピーカー側と干渉し、実機ではディスプレイ基板を少し切り取って逃がしたため、次回はOLED位置をもう少し左へ寄せてスピーカーとの干渉余裕を取る。
- 現在の状態: パススルー対応の1S Li-ion充放電統合モジュールが届くまでは、TP4056 + XL6009E1 + 18650構成を実稼働状態として運用する。充電時はラジオ電源OFF、使用時は充電USBを外す運用を継続する。
